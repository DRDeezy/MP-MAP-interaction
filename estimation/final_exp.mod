...
exp(omega) = omega_ss * ( ( exp(steady_state(b_eeu)) * exp(Y) * (1 + steady_state(exp(r_bu))) ) / ( steady_state(Y) * b_eeu(-1) * (1+r_bu(-1)) ) )^omega_r 
 * ( exp(steady_state(deltae)) / (deltae) )^gamma;
...
%exp(omega) = omega_ss * (( exp(b_eeu)  / exp(steady_state(b_eeu)) )^(omega_r)) * (( exp(steady_state(deltae)) / exp(deltae) )^gamma);
