...
exp(omega) = omega_ss * ( ( exp(steady_state(b_eeu)) * exp(Y) * (1 +  r_bu_ss  ) ) / ( steady_state(exp(Y)) * exp(b_eeu(-1)) * (1+exp(r_bu(-1))) ) )^omega_r 
 * ( exp(steady_state(deltae)) / exp(deltae) )^gamma;
...
%exp(omega) = omega_ss * (( 1/exp(b_eeu) * exp(steady_state(b_eeu)) )^(omega_r)) * (( exp(steady_state(deltae)) / exp(deltae) )^gamma);
...
estimation(datafile='data/DYNARE_estimdata'
                ...
                ,prefilter    = 0
                ,prior_trunc  = 1e-14
                ,mh_replic    = 500 %put 500 for test and a big number (like 100000) when serious
