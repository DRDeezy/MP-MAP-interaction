...
log_c_e %8
@#endif
mk_y  ${mk_y}$ (long_name='Retailers mark-up') %9
...
phi_y    = 0;              % MP RULE parameter, to be changed for simulationsv
@#endif


%-----------Estimated values of parameters------------%

@#if estimated_param == 1
...
chi_w_b  = 0.0;
%Parameters related to LAW MP
phi_B    =-0.1;           % Indebtness, less than 1
phi_AP   = 0.0;            % Asset prices
@#endif
...
@#if occbin == 1
occbin_constraints;
name 'BC'; bind s_e<0; relax log_b_ee > log(m_e_ss) + log_q_k_exp + log(1-deltak) + log_k_e - log_one_plus_r_b; 
end;
@#endif
...
shocks;
var e_A_e            =  (0.0005)^2; %Small shocks, o/w negative values for endogenous parameters OR need to reestimate parameters
%var e_A_e            =  (0.0072)^2;
%var e_mk_y           =  (0.2572)^2;
...
% @#if welfare_analysis == 0
% options_.noprint=0;
% options_.nofunctions=0;
% options_.nograph=0;
% stoch_simul(order=1, irf=200, periods=10000) W_EMU;
% @#endif
...
@#if occbin == 1
    ...
    var e_mk_y;
    periods 1:9;%, 10:15, 16:21, 22:27;% 10, 50, 90, 130, 131:169;
    values 0.5;%, 0.7, 0.3, 0;% -0.01,-0.02, 0.01, 0.02, 0;
end;
...
%Does not include output gap for now
@#if policy_regime == 0
    options_.policy_regime=0;
    x_start=[0.5,2.5]; %use optimal values as starting point
    A = []; %arguments for fmincon
    b = []; %arguments for fmincon
    Aeq = []; %arguments for fmincon
    beq = []; %arguments for fmincon
    lb = [0,1]; %sets lower and upper bounds for policy rule coefficients
    ub = [1,5];
    ...
    final_mp_only_param=xhat;
    save ('final_mp_only_param','final_mp_only_param'); %saves optimal policy coefficients
    @#endif
...
@#if policy_regime == 1
    ...
    [xhat,fhat] = fmincon(@welfare_objective,x_start,A,b,Aeq,beq,lb,ub); %calls separate file to solve for optimal policy with declared arguments
    %structure above returns argument xhat and value fhat of the function f(x)
    active_map_param=xhat;
    save ('active_map_param','active_map_param'); %saves optimal policy coefficients
    @#endif
...
% = 2 MP + Time varying LTV cap 
@#if policy_regime == 2
    options_.policy_regime=2;
    x_start=[-0.5]; %use optimal values as starting point
    A = []; %arguments for fmincon
    b = []; %arguments for fmincon
    Aeq = []; %arguments for fmincon
    beq = []; %arguments for fmincon
    lb = [-5]; %sets lower and upper bounds for policy rule coefficients
    ub = [0];
    [xhat,fhat] = fmincon(@welfare_objective,x_start,A,b,Aeq,beq,lb,ub); %calls separate file to solve for optimal policy with declared arguments
    %structure above returns argument xhat and value fhat of the function f(x)
    ltv_cap=xhat;
    save ('ltv_cap','ltv_cap'); %saves optimal policy coefficients
    @#endif


% = 3 MP + Time varying risk weights
@#if policy_regime == 3
    options_.policy_regime=3;
    x_start=[0.5,-10,0.5,-10]; %use optimal values as starting point
    A = []; %arguments for fmincon
    b = []; %arguments for fmincon
    Aeq = []; %arguments for fmincon
    beq = []; %arguments for fmincon
    lb = [0,-20,0,-20]; %sets lower and upper bounds for policy rule coefficients
    ub = [1,0,1,0];
    [xhat,fhat] = fmincon(@welfare_objective,x_start,A,b,Aeq,beq,lb,ub); %calls separate file to solve for optimal policy with declared arguments
    %structure above returns argument xhat and value fhat of the function f(x)
    weights_param=xhat;
    save ('weights_param','weights_param'); %saves optimal policy coefficients
    @#endif
...
% = 4 Lean against the wind MP: total borrowing
@#if policy_regime == 4
    options_.policy_regime=4;
    x_start=[0.8,2.35,0.0]; %use optimal values as starting point
    A = []; %arguments for fmincon
    b = []; %arguments for fmincon
    Aeq = []; %arguments for fmincon
    beq = []; %arguments for fmincon
    lb = [0,0,-0.5]; %sets lower and upper bounds for policy rule coefficients
    ub = [1,5,0];
    [xhat,fhat] = fmincon(@welfare_objective,x_start,A,b,Aeq,beq,lb,ub); %calls separate file to solve for optimal policy with declared arguments
    %structure above returns argument xhat and value fhat of the function f(x)
    law_borrowing=xhat;
    save ('law_borrowing','law_borrowing'); %saves optimal policy coefficients
    @#endif
    ...
    % = 5 Lean against the wind MP: asset prices
@#if policy_regime == 5
    options_.policy_regime=5;
    x_start=[0.8,1.6,3]; %use optimal values as starting point
    A = []; %arguments for fmincon
    b = []; %arguments for fmincon
    Aeq = []; %arguments for fmincon
    beq = []; %arguments for fmincon
    lb = [0,1,0]; %sets lower and upper bounds for policy rule coefficients
    ub = [1,5,10];
    [xhat,fhat] = fmincon(@welfare_objective,x_start,A,b,Aeq,beq,lb,ub); %calls separate file to solve for optimal policy with declared arguments
    %structure above returns argument xhat and value fhat of the function f(x)
    law_asset_prices=xhat;
    save ('law_asset_prices','law_asset_prices'); %saves optimal policy coefficients
    @#endif
...
@#if policy_regime == 0
    ...
    lb = [0,1]; %sets lower and upper bounds for policy rule coefficients
    ub = [1,5];
    [xhat,fhat] = fmincon(@welfare_objective_coop,x_start,A,b,Aeq,beq,lb,ub); %calls separate file to solve for optimal policy with declared arguments
    %structure above returns argument xhat and value fhat of the function f(x)
    final_mp_only_param_coop=xhat;
    save ('final_mp_only_param_coop','final_mp_only_param_coop'); %saves optimal policy coefficients
    @#endif
...
@#if policy_regime == 1
    options_.policy_regime=1;
    x_start=[0.8,2.35,0.8,2.35]; %use optimal values as starting point
    A = []; %arguments for fmincon
    b = []; %arguments for fmincon
    Aeq = []; %arguments for fmincon
    beq = []; %arguments for fmincon
    lb = [0,0,0,1]; %sets lower and upper bounds for policy rule coefficients
    ub = [1,5,1,5];
    [xhat,fhat] = fmincon(@welfare_objective_coop,x_start,A,b,Aeq,beq,lb,ub); %calls separate file to solve for optimal policy with declared arguments
    %structure above returns argument xhat and value fhat of the function f(x)
    active_map_param_coop=xhat;
    save ('active_map_param_coop','active_map_param_coop'); %saves optimal policy coefficients
    @#endif
...
@#if policy_regime == 2
    options_.policy_regime=2;
    x_start=[-0.089,0.65,1.68]; %use optimal values as starting point
    A = []; %arguments for fmincon
    b = []; %arguments for fmincon
    Aeq = []; %arguments for fmincon
    beq = []; %arguments for fmincon
    lb = [-5,0,1]; %sets lower and upper bounds for policy rule coefficients
    ub = [0,1,5];
    [xhat,fhat] = fmincon(@welfare_objective_coop,x_start,A,b,Aeq,beq,lb,ub); %calls separate file to solve for optimal policy with declared arguments
    %structure above returns argument xhat and value fhat of the function f(x)
    ltv_cap_coop=xhat;
    save ('ltv_cap_coop','ltv_cap_coop'); %saves optimal policy coefficients
    @#endif
...
@#if policy_regime == 3
    options_.policy_regime=3;
    x_start=[0.5,-10,0.5,-10,0.8,2.35]; %use optimal values as starting point
    A = []; %arguments for fmincon
    b = []; %arguments for fmincon
    Aeq = []; %arguments for fmincon
    beq = []; %arguments for fmincon
    lb = [0,-20,0,-20,0,1]; %sets lower and upper bounds for policy rule coefficients
    ub = [1,0,1,0,1,5];
    [xhat,fhat] = fmincon(@welfare_objective_coop,x_start,A,b,Aeq,beq,lb,ub); %calls separate file to solve for optimal policy with declared arguments
    %structure above returns argument xhat and value fhat of the function f(x)
    weights_param_coop=xhat;
    save ('weights_param_coop','weights_param_coop'); %saves optimal policy coefficients
    @#endif



%Actually 4 and 5 are redundant
% = 4 Lean against the wind MP: total borrowing
@#if policy_regime == 4
    options_.policy_regime=4;
    x_start=[0.8,2.35,-0.5]; %use optimal values as starting point
    A = []; %arguments for fmincon
    b = []; %arguments for fmincon
    Aeq = []; %arguments for fmincon
    beq = []; %arguments for fmincon
    lb = [0,0,-5]; %sets lower and upper bounds for policy rule coefficients
    ub = [1,5,0];
    [xhat,fhat] = fmincon(@welfare_objective_coop,x_start,A,b,Aeq,beq,lb,ub); %calls separate file to solve for optimal policy with declared arguments
    %structure above returns argument xhat and value fhat of the function f(x)
    law_borrowing_coop=xhat;
    save ('law_borrowing_coop','law_borrowing_coop'); %saves optimal policy coefficients
    @#endif
...
@#if policy_regime == 5
    options_.policy_regime=5;
    x_start=[0.8,2.35,5]; %use optimal values as starting point
    A = []; %arguments for fmincon
    b = []; %arguments for fmincon
    Aeq = []; %arguments for fmincon
    beq = []; %arguments for fmincon
    ...
    save ('law_asset_prices_coop','law_asset_prices_coop'); %saves optimal policy coefficients
    @#endif
