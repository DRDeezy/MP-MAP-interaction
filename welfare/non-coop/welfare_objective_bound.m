function [fval]=welfare_objective_bound(xopt)
...
switch options_.policy_regime
    ...
    case 0
        ...
        if xopt(1)>=1 || xopt(1)<=0 || xopt(2)>=5 || xopt(2)<=1 %|| xopt(3)>=2 || xopt(3)<=-2
            fval=150+sum(xopt.^2);
            return
        end
    ...
    case 1
        ...
        if xopt(1)>=1 || xopt(1)<=0 || xopt(2)>=5 || xopt(2)<=0
            fval=150+sum(xopt.^2);
            return
        end
    ...
    case 2
        ...
        [info, oo_, options_] = stoch_simul(M_, options_, oo_, var_list_); %get decision rules and moments
        if info(1) %solution was not successful
            fval=150+sum([xopt(1)].^2); %return with penalty
        else
            fval=-oo_.mean;
        end
    ...
    case 3
        ...
        if xopt(1)>=1 || xopt(1)<=0 || xopt(2)>=0 || xopt(2)<=-20 || xopt(3)>=1 || xopt(3)<=0 || xopt(4)>=0 || xopt(4)<=-20
            fval=150+sum(xopt.^2);
            return
        end
    ...
    case 4
        ...
        if xopt(1)>=1 || xopt(1)<=0 || xopt(2)>=5 || xopt(2)<=1 || xopt(3)>=0 || xopt(3)<=-0.5 %|| xopt(4)>=2 || xopt(4)<=-2
            fval=150+sum(xopt.^2);
            return
        end
